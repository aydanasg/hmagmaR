...
AnnotationFileHmagma <- function(hic, regulatoryRegions = NULL, promoterRegions = NULL, enhancerRegions = NULL, snps, annotated_genes,  snpgeneexon, AnnotationFile) {
  ...
  # Check if either regulatoryRegions or both promoterRegions and enhancerRegions are provided
  if (!is.null(regulatoryRegions)) {
    message("Using regulatoryRegions...")
    # Add logic for when regulatoryRegions is used
  ...
  #Selecting exonic/promoter-interacting SNPS overlapping with enhancerRegions  

  enhancerRegions<- enhancerRegions
  colnames(enhancerRegions)<-c("chr", "start", "end")
  enhancerRegions_ranges<-GRanges(enhancerRegions$chr, IRanges(as.numeric(enhancerRegions$start), as.numeric(enhancerRegions$end)))   
   
  olap<-findOverlaps(query = snpint, subject = enhancerRegions_ranges) #default maxgap=-1L, when one range's start/end strictly inside the other, the gap is considered to be -1.
  regulatoryRegions_snp_ranges<-snpint[queryHits(olap)]
  mcols(regulatoryRegions_snp_ranges)<-cbind(mcols(snpint[queryHits(olap)]), mcols(enhancerRegions_ranges[subjectHits(olap)]))
